...
from keyboard import (
    MAIN_KEYBOARD,
    ops_keyboard, users_keyboard, analytics_keyboard,
    actions_keyboard, alerts_keyboard_with_state,
    reinstall_keyboard, node_picker_keyboard,
    confirm_keyboard, back_keyboard,
)
from nodes import (
    format_status, restart_by_name, restart_all_nodes,
    panic_restart, get_nodes_inline_list, get_nodes,
)
from users import (
    format_users, create_user, delete_user,
    disable_user, enable_user,
    get_user_qr, get_user_subscription,
    get_user_info, reset_user_traffic,
    set_traffic_limit, extend_expiry, set_max_devices,
)
from traffic import (
    traffic_stats, node_stats_by_pattern,
    online_clients, top_users_by_traffic, get_nodes_for_chart,
)
...
# Bottom-keyboard button texts that map to quick actions
_QUICK_BUTTONS = {
    "📡 Статус":          "quick_status",
    "➕ Добавить юзера":  "quick_adduser",
    "📅 Отчёт":           "quick_report",
}
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "  📡 Статус — текущий статус нод\n"
        "  ➕ Добавить юзера — создать нового\n"
        "  📅 Отчёт — сводка прямо сейчас\n\n"
        "*Разделы:*\n"
        "  🧠 OPS — ноды, серверы, SSH, ребут\n"
        "  👥 Юзеры — полное управление\n"
        "  📊 Аналитика — трафик, топы, графики\n"
        "  ⚡ Действия — рестарты, синх, бэкап\n"
        ...
        "  • Диск >85% / CPU >90%\n"
        "  • Истечение юзеров / лимит трафика\n"
...
# ── Main text-button router ────────────────────────────────────────────────────

async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await guard(update):
        return

    # If we're mid-conversation, route to input handler
    ...
    elif text == "➕ Добавить юзера":
        context.user_data["conv_state"] = WAIT_CREATE
        await update.message.reply_text(
            "➕  *Создать юзера*\n\n"
            "Введи имя и параметры в одну строку:\n"
            "`username` — базовый\n"
    ...
    # ── Section buttons ────────────────────────────────────────────────────────
        ...
        await update.message.reply_text(
            "👥  *Управление пользователями*", parse_mode="Markdown",
            reply_markup=users_keyboard(),
        )

    elif text == "📊 Аналитика":
        await update.message.reply_text(
            "📊  *Аналитика*", parse_mode="Markdown",
            reply_markup=analytics_keyboard(),
        )

    elif text == "⚡ Действия":
        await update.message.reply_text(
            "⚡  *Быстрые действия*", parse_mode="Markdown",
        ...
        await update.message.reply_text(
            ...
            f"⏱  Проб:      каждые 5 сек\n"
            f"🔍  Фильтр:    2 из 3\n"
            f"📅  Daily rep: {'✅ включён' if _sched.daily_report_enabled else '❌ выключен'}\n\n"
            "Нажми /start для сброса",
            parse_mode="Markdown",
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("ops_status", "ops_refresh"):
        ...
        await query.edit_message_text("🖥  Запрашиваю статистику серверов по SSH…")
        await query.edit_message_text(format_server_stats(), reply_markup=ops_keyboard())

    elif data == "ops_heal":
        # Auto-heal: the probe loop already restarts downed nodes.
        # We just show current status as confirmation.
        await query.edit_message_text(
            "🚨  *Auto-Heal*\n\nПорт-монитор работает в фоне.\n"
            "При падении ноды авто-рестарт запускается автоматически.\n\n"
            + format_status(),
            parse_mode="Markdown",
            reply_markup=ops_keyboard(),
        )

    elif data == "ops_panic":
        await query.edit_message_text(
            "⚠️  *PANIC RESTART*\n\n"
            "Stop → 2s → Start ALL сервисов на ВСЕХ нодах.\n\nПодтверди:",
            parse_mode="Markdown",
            reply_markup=confirm_keyboard("panic", "all"),
        )
        ...
        # ssh_node_<node_id>_<ntype>  — we only need node_id
        # node_id can contain underscores, ntype is last segment
        parts  = data[len("ssh_node_"):].rsplit("_", 1)
        node_id = parts[0]
        context.user_data["ssh_target_node"] = node_id
        context.user_data["conv_state"] = WAIT_SSH_CMD
        await query.edit_message_text("💻  Введи bash-команду для выполнения на ноде:")

    elif data == "ops_reboot_menu":
        nodes = get_nodes_inline_list()
        await query.edit_message_text(
            "♻️  *Ребут сервера*\n\nВыбери ноду (перезагрузит физический сервер):",
            parse_mode="Markdown",
            reply_markup=node_picker_keyboard("reboot_node", nodes),
        )
        ...
        await query.edit_message_text(
            ...
            "Все сервисы кратковременно упадут.",
            parse_mode="Markdown",
            reply_markup=confirm_keyboard("reboot_srv", node_id),
        )
    ...
    # ══ USERS ══════════════════════════════════════════════════════════════════
        ...
        await query.edit_message_text(
            "📅  *Продлить срок*\n\n"
            "Формат: `имя дней`\n"
            "Пример: `john 30` — добавит 30 дней",
            parse_mode="Markdown",
        )
        ...
        await query.edit_message_text(
            ...
            "`john 3` — 3 устройства  |  `john 0` — без лимита",
            parse_mode="Markdown",
        )
    ...
    # ══ ACTIONS ════════════════════════════════════════════════════════════════
    ...
    elif data == "action_restart_fi":
        await query.edit_message_text("🇫🇮  Перезапускаю FI ноды…")
        await query.edit_message_text(
            "🇫🇮  FI Restart\n\n" + restart_by_name("fi"),
            reply_markup=actions_keyboard(),
        )

    elif data == "action_restart_ge":
        await query.edit_message_text("🇩🇪  Перезапускаю GE ноды…")
        await query.edit_message_text(
            "🇩🇪  GE Restart\n\n" + restart_by_name("ge"),
            reply_markup=actions_keyboard(),
        )
        ...
        await query.edit_message_text(
            "🔄  Перезапустить ВСЕ ноды?\n\nВсе сервисы будут перезапущены.",
        ...
        await query.edit_message_text(restart_all_nodes(), reply_markup=actions_keyboard())

    elif data == "action_sync":
        await query.edit_message_text("🔁  Синхронизирую ноды…")
        from mcp import mcp as _mcp
        r   = _mcp("sync")
        msg = "✅  Синхронизация запущена" if "error" not in r else f"❌  Ошибка синх: {r['error']}"
        await query.edit_message_text(msg, reply_markup=actions_keyboard())

    elif data == "action_backup":
        await query.edit_message_text("💾  Снимаю бэкап конфигов со всех нод…")
    ...
    # ══ ALERTS ═════════════════════════════════════════════════════════════════
        ...
        mute_alerts(1)
        await query.edit_message_text("🔇  Алерты отключены на 1 час",
                                      reply_markup=alerts_keyboard_with_state(_sched.daily_report_enabled))

    elif data == "alerts_mute_3":
        mute_alerts(3)
        await query.edit_message_text("🔇  Алерты отключены на 3 часа",
                                      reply_markup=alerts_keyboard_with_state(_sched.daily_report_enabled))

    elif data == "alerts_mute_6":
        mute_alerts(6)
        await query.edit_message_text("🔇  Алерты отключены на 6 часов",
                                      reply_markup=alerts_keyboard_with_state(_sched.daily_report_enabled))
...
async def _input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Any main-keyboard button cancels the current state
    if text in _QUICK_BUTTONS or text in _SECTION_BUTTONS:
        context.user_data.clear()
        await menu_handler(update, context)
        return

    context.user_data["conv_state"] = None   # clear before processing

    # ── SSH node-picker fallback ───────────────────────────────────────────────
    if state == WAIT_SSH_NODE:
        await update.message.reply_text(
            "👆  Выбери ноду кнопкой выше, затем введи команду",
            reply_markup=MAIN_KEYBOARD,
        )
        return
    ...
    if state == WAIT_SSH_CMD:
        ...
        if not node_id:
            await update.message.reply_text("❌  Нода не выбрана — начни сначала")
            return
    ...
    if state == WAIT_CREATE:
        ...
        await update.message.reply_text(
            get_user_info(text), parse_mode="Markdown", reply_markup=users_keyboard()
        )
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(err, (Conflict, NetworkError, TimedOut)):
        logging.warning(f"[BOT] transient: {err}")
        return
