...
def format_status() -> str:
    ...
    for n in stats.get("nodes", {}).get("list", []):
        clients_map[n["name"]] = n.get("online", 0)
    ...
    for node in nodes:
        ...
        ip     = node.get("ip", "?")
        port   = node.get("port", "?")
        clients = clients_map.get(name, 0)
        ...
        status_s = "online" if status == "online" else "OFFLINE"

        lines.append(
            f"\n{em} {name}  [{tag}]  {status_s}\n"
            f"   📍 {ip}:{port}\n"
            f"   👥 {clients} clients  "
            f"⬇ {rx_gb} GB  ⬆ {tx_gb} GB"
        )
...
def restart_service(node_id: str, service: str) -> dict:
    ...
    return mcp("execute_ssh", {"nodeId": node_id, "command": commands.get(service, commands["all"])})
...
def restart_by_name(name_pattern: str, service: str = "all") -> str:
    ...
    for node in targets:
        ...
        (err_list if isinstance(r, dict) and "error" in r else ok_list).append(node["name"])
...
def restart_all_nodes() -> str:
    ...
    for node in nodes:
        ...
        em = "✅" if not (isinstance(r, dict) and "error" in r) else "❌"
...
def panic_restart() -> str:
    ...
    for node in nodes:
        ...
        r = mcp("execute_ssh", {"nodeId": node["_id"], "command": cmd})
        em = "✅" if not (isinstance(r, dict) and "error" in r) else "❌"
        lines.append(f"{em} {node['name']}")
